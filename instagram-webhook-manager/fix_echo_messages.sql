-- Fix trigger to handle echo messages (business sent messages)
...
-- Echo 메시지도 message_count 증가시키는 트리거 함수 수정
...
BEGIN
    ...
    -- 메시지 카운트 증가 (이미 메인 트리거에서 처리하므로 중복 방지)
    -- 이 트리거는 백업용으로 유지
